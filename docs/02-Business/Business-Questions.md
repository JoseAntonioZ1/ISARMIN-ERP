...
**Actualización 6 — 2026-07-19 (análisis previo al módulo de Autenticación, Fase 4):** al analizar `Use-Cases.md` (UC-01) contra el esquema físico, se detectó que `Physical-Data-Model.md`/`Data-Dictionary.md` nunca definieron un campo de acceso (login) distinto del nombre completo, pese a que `API-Design.md` ya asumía uno (`{ usuario, credencial }`). Se resolvieron **BQ-047** y **BQ-048** con recomendaciones técnicas documentadas (RN-036 a RN-038 nuevas en `Business-Rules.md`): campo `nombre_usuario` único para login, bloqueo temporal de 15 minutos tras 5 intentos fallidos sin expiración forzada de contraseña, y restablecimiento exclusivo del Administrador (sin autoservicio por correo, por no existir un servicio de envío de correo en `TECH_STACK.md`). Ambas quedan marcadas **🔵 recomendación documentada**, pendientes de validación final con ISARMIN.
...
| BQ-047 | 🔵 **RECOMENDACIÓN DOCUMENTADA.** ¿Qué política de seguridad de contraseñas se requiere (complejidad, expiración, bloqueo tras intentos fallidos)? | Media | RF-006, Non-Functional-Requirements.md (RNF-013) |
| BQ-048 | 🔵 **RECOMENDACIÓN DOCUMENTADA.** ¿El restablecimiento de contraseña debe ser autoservicio (ej. correo de recuperación) o exclusivamente gestionado por el Administrador? | Media | RF-007 |
...
>
> **Recomendación técnica a BQ-047 y BQ-048 (análisis previo al módulo de Autenticación, 2026-07-19):** ante la falta de política definida, se documentan como valores por defecto razonados (no confirmados por ISARMIN): (1) bloqueo temporal de 15 minutos tras 5 intentos fallidos, sin expiración forzada de contraseña (RN-037); (2) restablecimiento de contraseña exclusivo del Administrador, sin autoservicio por correo, ya que `TECH_STACK.md` no incluye un servicio de envío de correo (RN-038). Además, se detectó que ningún documento había definido un campo de login distinto del nombre completo — se documenta `nombre_usuario` como campo de acceso único (RN-036). Los tres puntos quedan marcados **[PV]** en `Business-Rules.md`, pendientes de validación final con ISARMIN antes de producción.
